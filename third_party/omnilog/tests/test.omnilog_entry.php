...
class Test_omnilog_entry extends Testee_unit_test_case {
  ...
  {
    $s = $this->_subject;
    $v = $this->_props['addon_name'];

    $this->assertIdentical($v, $s->set_addon_name(123));
    $this->assertIdentical($v, $s->set_addon_name(FALSE));
    $this->assertIdentical($v, $s->set_addon_name(new StdClass()));
    $this->assertIdentical($v, $s->set_addon_name(NULL));
  }
  ...
  public function test__set_date__invalid_values()
  {
    ...
    $this->assertIdentical($v, $s->set_date('Invalid'));
    $this->assertIdentical($v, $s->set_date(FALSE));
    $this->assertIdentical($v, $s->set_date(new StdClass()));
    $this->assertIdentical($v, $s->set_date(NULL));
    $this->assertIdentical($v, $s->set_date(time() + 100));
    $this->assertIdentical($v, $s->set_date(-100));
  }
  ...
  {
    $s = $this->_subject;
    $v = $this->_props['log_entry_id'];

    $this->assertIdentical($v, $s->set_log_entry_id('Invalid'));
    $this->assertIdentical($v, $s->set_log_entry_id(FALSE));
    $this->assertIdentical($v, $s->set_log_entry_id(new StdClass()));
    $this->assertIdentical($v, $s->set_log_entry_id(NULL));
    $this->assertIdentical($v, $s->set_log_entry_id(0));
  }
  ...
  {
    $s = $this->_subject;
    $v = $this->_props['message'];

    $this->assertIdentical($v, $s->set_message(123));
    $this->assertIdentical($v, $s->set_message(FALSE));
    $this->assertIdentical($v, $s->set_message(new StdClass()));
    $this->assertIdentical($v, $s->set_message(NULL));
  }
  ...
  {
    $s = $this->_subject;
    $v = $this->_props['extended_data'];

    $this->assertIdentical($v, $s->set_extended_data(123));
    $this->assertIdentical($v, $s->set_extended_data(FALSE));
    $this->assertIdentical($v, $s->set_extended_data(new StdClass()));
    $this->assertIdentical($v, $s->set_extended_data(NULL));
  }
  ...
  {
    $s = $this->_subject;
    $v = $this->_props['notify_admin'];

    $this->assertIdentical($v, $s->set_notify_admin(123));
    $this->assertIdentical($v, $s->set_notify_admin('Invalid'));
    $this->assertIdentical($v, $s->set_notify_admin(new StdClass()));
    $this->assertIdentical($v, $s->set_notify_admin(NULL));
  }
  ...
  public function test__set_type__invalid_values()
  {
    ...
    $this->assertIdentical($v, $s->set_type('Invalid'));
    $this->assertIdentical($v, $s->set_type(FALSE));
    $this->assertIdentical($v, $s->set_type(new StdClass()));
    $this->assertIdentical($v, $s->set_type(NULL));
    $this->assertIdentical($v, $s->set_type(20));
  }
